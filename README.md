...
1. 下载 [ShadowsocksR](https://xn--nos809b.com/ssr-download/ssr-win.7z)，解压并运行。

2. 右键`ShadowsocksR`托盘图标，点击`SSR服务器订阅 -> SSR服务器订阅设置`。

3. 找到 [](https://xn--nos809b.com/user) 的`All-in-One`模块，点击`复制订阅地址到剪切板`，填进`地址`，点击`Add`，点击`确定`（请确定左边列表中是否显示有“https://xn--nos809b.com/xxxxxx“，如果没有请再次添加）。
...
1. 下载 [ShadowsocksR-Tap](https://xn--nos809b.com/ssr-download/ssr-tap.zip)，解压并运行。

2. 打开[墙洞的节点列表](https://xn--nos809b.com/user/node)，任意选择一条低延迟的节点，复制`配置链接`。
...
1. 下载 [ShadowsocksX](https://xn--nos809b.com/ssr-download/ssr-mac.dmg)，拖至`应用程序`文件夹内，运行。

2. 点击`ShadowsocksR`托盘图标，点击`服务器 -> 编辑订阅`。

3. 点击`+`，找到 [墙洞](https://xn--nos809b.com/user) 的`All-in-One`模块，点击`复制订阅地址到剪切板`，填进`订阅地址`，点击`OK`。
...
2. 打开`Shadowrkcoet`，点击首页的右上角的`+`，选择类型：`Subscribe`，找到 [墙洞](https://xn--nos809b.com/user) 的`All-in-One`模块，点击`复制订阅地址到剪切板`，将复制的内容填入`URL`后面的编辑框，点击`完成`。
...
2. Get：[🐰 User Data](https://workflow.is/workflows/890b53e8519a42009609811696d5dca7) 和 [Rule OTA](https://workflow.is/workflows/25bb170a015941948cefc6ea196f481c)

3. 将 [墙洞](https://xn--nos809b.com/user) 的邮箱/密码填入`🐰 User Data`点击正上方的`▶️`等待流程运行完毕导出至相对应程序即可。

---

#### Android：

1. 下载 [SSR](https://xn--nos809b.com/ssr-download/ssr-android.apk)，安装并运行。

2. 点击应用顶部的`ShadowsocksR ⬇️` -> `➕` -> `添加/升级 SSR 订阅`

3. 找到 [墙洞](https://xn--nos809b.com/user) 的`All-in-One`模块，点击`复制订阅地址到剪切板`，填进去并添加即可。
